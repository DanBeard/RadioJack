...
size_t MeshtasticWifiBridgeClient::write(const uint8_t *buf, size_t size) {
  ...
  meshPacket.to = MESH_DEST_ADDR; // this should be filled in by the radio right?
  meshPacket.channel = MESH_CHANNEL_IDX;
...
String MeshtasticWifiBridgeClient::readString() {
  if (!connected()) {
...
void MeshtasticWifiBridgeClient::requestConfigInfo() {
  meshtastic_ToRadio toRadio = meshtastic_ToRadio_init_default;
...
void MeshtasticWifiBridgeClient::debug(char * msg) {
  write_to_screen(msg);
